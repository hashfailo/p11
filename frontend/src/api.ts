...
export async function uploadFile(file: File): Promise<{ lore: Lore; source_length: number; session_id: string; warning?: string }> {
  const form = new FormData();
  form.append('file', file);
  const res = await fetch(`${BASE}/upload`, { method: 'POST', body: form, headers: sessionHeaders() });
  if (!res.ok) {
