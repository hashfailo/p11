        ...
        <header className="sticky top-0 z-50 border-b border-white/[0.06] bg-ink/95 px-4 py-3 backdrop-blur-md sm:px-8">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
            <button
              ...
              className="flex items-center gap-2 text-sm font-semibold tracking-wide text-slate-300 transition hover:text-white"
            >
              ...
              {steps.map((item, index) => {
                ...
                return (
                  ...
                  >
                    {index > 0 && (
                      <div
                        className={`h-px w-3 sm:w-8 ${complete ? "bg-accent/60" : "bg-white/[0.08]"}`}
                      />
                    )}
                    <div
                      className={`flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-[11px] font-semibold uppercase tracking-wider transition-all duration-300 sm:px-3 ${active ? "bg-accent text-ink shadow-neumorphic" : complete ? "text-accent" : "text-slate-600"}`}
                    >
                      {complete && !active ? (
      ...
      <main className={step !== "upload" ? "animate-fade-in" : ""}>
        ...
        {step === "diverge" && branch && (
          <button
            ...
            className="fixed bottom-5 right-5 z-40 flex items-center gap-2 rounded-xl bg-accent px-5 py-3 text-sm font-bold text-ink shadow-neumorphic transition duration-300 hover:brightness-110 active:scale-[0.98]"
          >
